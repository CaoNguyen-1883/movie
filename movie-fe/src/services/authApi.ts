...
import type { AuthResponse, ChangePasswordPayload, RefreshAuthResponse } from '@/types/auth';
...
export const refreshAuth = async (payload: { refreshToken: string }): Promise<RefreshAuthResponse> => {
  const response = await api.post<RefreshAuthResponse>('/auth/refresh-token', payload);
  return response.data;
};
