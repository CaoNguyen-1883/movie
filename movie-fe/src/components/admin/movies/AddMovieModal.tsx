...
type AddMovieFormData = z.infer<typeof movieSchema>;
...
export function AddMovieModal({ isOpen, onClose }: AddMovieModalProps) {
  ...
  const {
    fields: backdropFields,
    append: appendBackdrop,
    remove: removeBackdrop,
  } = useFieldArray({
    control,
    name: 'backdropUrls',
  });
  ...
  const mutation = useMutation({
    mutationFn: (newMovie: CreateMoviePayload) => createMovie(newMovie),
    onSuccess: () => {
  ...
  const onSubmit = (data: AddMovieFormData) => {
    const submissionData: CreateMoviePayload = {
      ...data,
      genres: data.genres ?? [],
  ...
  const genreOptions: MultiSelectOption[] = React.useMemo(
    () => genres?.map((g) => ({ value: g._id, label: g.name })) ?? [],
    [genres],
  );

  const peopleOptions: MultiSelectOption[] = React.useMemo(
